  ...
  fetch(`/api/reports/player/${played_reportId}`, {
    ...
    .then(report => {
      ...
      const accuracy = Math.floor(
        (report.correct / report.questions.length) * 100
      );
      ...
      document.querySelector(
        "#totalCorrects"
      ).textContent = `${report.correct}`;
      document.querySelector(
        "#totalIncorrects"
      ).textContent = `${report.incorrect}`;
      document.querySelector(
        "#totalUnattempted"
      ).textContent = `${report.unattempted}`;
